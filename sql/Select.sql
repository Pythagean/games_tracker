...
select s.*, g.title from sessions s join games g on s.game_id = g.game_id order by start_date asc;

select s.*, g.title, p.name
from sessions s 
left join games g on s.game_id = g.game_id 
left join session_player sp on s.session_id = sp.session_id
left join players p on sp.player_id = p.player_id
where g.title = 'Risk of Rain 2'
